...
export function CreateSchemaDrawer({
  open,
  onClose,
  onCreated,
      ...
      <Form form={form} layout="vertical" component="div">
        <Form.Item
          ...
          rules={[
            ...
            {
              pattern: /^[A-Z0-9-]+$/,
              message: "Chỉ gồm chữ hoa, số và dấu gạch ngang",
            },
          ]}
        >
          <Input
            placeholder="VD: VAT-INVOICE"
            onChange={(e) =>
        ...
        <Form.Item name="description" label="Mô tả / Hướng dẫn cho AI">
          <Input.TextArea
            rows={3}
            placeholder="Mô tả đặc điểm của loại chứng từ này..."
          />
        </Form.Item>

        <Divider plain>
          <span className="text-xs font-medium text-gray-400 dark:text-[#8b949e]">
            Trường dữ liệu (tuỳ chọn)
          </span>
        </Divider>
          ...
          {(items, { add, remove }) => (
            <div className="space-y-3">
              {items.map(({ key, name }) => (
                <div
                  key={key}
                  className="bg-gray-50 dark:bg-[#161b22] border border-gray-200 dark:border-[#30363d] rounded-lg p-3"
                >
              ...
              <Button
                ...
                icon={<Plus size={13} />}
                onClick={() =>
                  add({ dataType: "TEXT", position: "BODY", isRequired: false })
                }
              >
                Thêm trường
              </Button>
            ...
            <div className="space-y-3">
              {items.map(({ key, name: tName }) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-gray-500 dark:text-[#8b949e] mb-1 mt-2">
                    Cột trong bảng
                  </p>
